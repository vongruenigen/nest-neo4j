    ...
    return {
      ...
      providers: [
        ...
        },
        {
          ...
          useFactory: async (connection: Neo4jConnection) =>
          ...
          useFactory: (configService: ConfigService): Neo4jConnection => ({
            ...
            config: {
              ...
              trustedCertificates: configService
                .get("NEO4J_TRUSTED_CERTIFICATES")
                ?.split(",")
                .map((cert) => cert.trim()),
              fetchSize: coerceNumber(configService.get("NEO4J_FETCH_SIZE")),
              maxConnectionPoolSize: coerceNumber(
                configService.get("NEO4J_MAXIMUM_POOL_SIZE"),
              ),
              maxTransactionRetryTime: coerceNumber(
                configService.get("NEO4J_TRANSACTION_RETRY_TIME"),
              ),
              maxConnectionLifetime: coerceNumber(
                configService.get("NEO4J_MAX_CONNECTION_LIFETIME"),
              ),
              connectionAcquisitionTimeout: coerceNumber(
                configService.get("NEO4J_CONNECTION_ACQUISITION_TIMEOUT"),
              ),
              connectionTimeout: coerceNumber(
                configService.get("NEO4J_CONNECTION_TIMEOUT"),
              ),
              disableLosslessIntegers: isTruthy(
                configService.get("NEO4J_DISABLE_LOSSLESS_INTEGERS"),
              ),
